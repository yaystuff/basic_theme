...
$request_ingredients = '';
if (isset($_GET['ing'])) {
	$ingredient_array = (array)$_GET["ing"];
	$request_ingredients = $_GET["ing"];
...
$search_tab = 1;
if (!empty($request_ingredients)) {
	$search_tab = 1;
} else if (!empty($request_term) || !empty($request_cook) || !empty($request_prep) || !empty($request_cook) || !empty($request_serv) || !empty($request_cat) || !empty($request_mc) || !empty($request_diff)) {
	$search_tab = 2;
}
	...
	<div class="row">
		...
		<section class="content <?php echo esc_attr($section_class); ?>">
			<div class="recipefinder">
				<nav class="tabs">
					<ul>
						<li class="<?php echo $search_tab == 1 ? 'active' : ''; ?>"><a href="#tab1" title="<?php _e('Search by ingredients', 'socialchef') ?>"><?php _e('Search by ingredients', 'socialchef') ?></a></li>
						<li class="<?php echo $search_tab == 2 ? 'active' : ''; ?>"><a href="#tab2" title="<?php _e('Classic search', 'socialchef') ?>"><?php _e('Classic search', 'socialchef') ?></a></li>
					</ul>
				</nav>
				<div class="tab-content" id="tab1" style="<?php echo $search_tab == 1 ? 'display: block;' : 'display:none'; ?>">
					<form id="custom-search-1" method="get" action="<?php echo $sc_theme_globals->get_search_form_page_url(); ?>">
						<div class="left">
							...
							<div class="ingredients">
								...
								<?php
									...
									foreach ($ingredient_terms as $term) {
										?>
										<a href="#" class="add_random_search_ingredient button gold"><?php echo esc_html( $term->name ); ?></a>
										<?php
									}
								?>
							</div>
				...
				<div class="tab-content" id="tab2" style="<?php echo $search_tab == 2 ? 'display: block;' : 'display:none'; ?>">
						...
						<div class="row">
							...
							<div class="one-fourth">
								<select name="mc" id="recipe_meal_course">
									...
									?>	
									<option <?php echo ($term_id == (int)$request_mc ? 'selected="selected"' : ''); ?> value="<?php echo esc_attr( $term_id ); ?>"><?php echo $term_name; ?></option>
									<?php	}
										}
									} ?>
								</select>

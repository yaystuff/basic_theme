...
<?php
	...
	$recipe_preparation_time = null;
	$recipe_cooking_time = null;
	$recipe_serving = null;
	$recipe_difficulty = null;
	$recipe_meal_course = null;
	$mf_recipe_cooking_time = null;
	$mf_recipe_preparation_time = null;
				...
				<article id="recipe-<?php the_ID(); ?>" class="recipe">
					<div class="row">
						...
						<div class="one-third entry-header">
							...
							<?php
							if ($sc_theme_globals->enable_ingredients() && count($ingredient_results) > 0) { ?>
							<dl class="ingredients">
								...
								foreach ($ingredient_results as $ingredient_result) {
									...
									echo "<dt>" . (!$sc_theme_globals->disable_amount_fractions() ? SocialChef_Theme_Utils::convert_decimal_to_fraction($ingredient_result->amount) : number_format_i18n($ingredient_result->amount, 2)) . " $unit_name</dt>";
							...
							<div class="favorite">
								...
								if ( $sc_recipes_post_type->is_marked_as_favorite($current_user->ID, $post->ID)) {
									?>
									<a href="javascript:void(0);" class="remove"><i class="fa fa-fw fa-heart"></i> <span><?php _e('Remove from favorites', 'socialchef'); ?></span></a>
									<?php
								} else { ?>
									<a href="javascript:void(0);"><i class="fa fa-fw fa-heart"></i> <span><?php _e('Add to favorites', 'socialchef'); ?></span></a>
								<?php } ?>
							</div>
							<?php } else { ?>
							<div class="favorite">
								<a class="disabled" href="<?php echo esc_url ( $sc_theme_globals->get_login_page_url() ); ?>"><i class="fa fa-fw fa-heart"></i> <span><?php _e('Add to favorites', 'socialchef'); ?></span></a>
							</div>
							<?php } ?>
							<?php } // defined('BP_VERSION')  ?>
							<div class="print">
								<a class="" onclick="window.print();" href="#"><i class="fa fa-fw fa-print"></i> <span><?php _e('Print recipe', 'socialchef'); ?></span></a>
							</div>							
						</div><!--// one-third -->
						<!--two-third-->
						<div class="two-third">
							...
							<?php } ?>
							<?php if (isset($recipe_instructions) && count($recipe_instructions) > 0) { ?>
							<div class="instructions" itemprop="recipeInstructions">
								<ol>
								<?php
								$instructions_array = unserialize($recipe_instructions);
								for ( $i = 0; $i < count($instructions_array); $i++ ) { 
									if (isset($instructions_array[$i]) && isset($instructions_array[$i]['instruction'])) {

...
if (is_user_logged_in()) {
	if ( current_user_can( 'manage_options' ) ) {
		...
		echo "<p class='error'>" . esc_html__('This page is not accessible when a user is logged in. You are seeing this warning because you have administrative privileges, otherwise you would be redirected to home. To access this page properly, please log out.', 'socialchef') . "</p>";
		echo '<a href="' . wp_logout_url() . '">' . esc_html__('Logout', 'socialchef') . '</a>';
...
if( isset( $_POST['log'] ) && wp_verify_nonce( $_POST['login_form_nonce'], 'login_form' ) ){
	...
	if ( !is_wp_error( $login ) ) {
		...
		do_action( 'wp_login', esc_attr($_POST['log']), $login );
